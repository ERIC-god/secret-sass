...
export default function AppSettingPage({params:{id}}:{params:{id:string}}) {
  const [appName,setAppName]=useState('')
  const [appDescription,setAppDescription]=useState('')
  const [isShowSuccess,setIsShowSucces]=useState(false)

  const {mutate:resetApp, isSuccess} = trpcClientReact.app.resetApp.useMutation()
  ...
  return (
    <div className="w-full flex justify-center mt-10">
      <div className="w-full max-w-2xl bg-gradient-to-br from-[#23235b] via-[#29295e] to-[#23235b] rounded-2xl shadow-xl border border-[#35356a] p-10">
        ...
        <div className="text-gray-400 mb-8">Configure your app's basic information.</div>
        {/* Basic Info */}
        <div className="mb-8">
          <div className="text-lg font-bold text-white mb-4">General</div>
          <form className="flex flex-col gap-6"   onSubmit={handleSubmit}>
            <div>
              <label className="block text-sm text-gray-300 mb-1" htmlFor="appName">App Name</label>
              <input
                id="appName"
                type="text"
            ...
            <div>
              <label className="block text-sm text-gray-300 mb-1" htmlFor="appUrl">App Description</label>
              <input
                id="appUrl"
                type="text"
        ...
        {/* Danger Zone */}
        <div className="mt-10">
          <div className="text-lg font-bold text-pink-400 mb-2">Danger Zone</div>
          <div className="text-gray-400 text-sm mb-3">This action is destructive and cannot be undone.</div>
          <Button
            variant="outline"
